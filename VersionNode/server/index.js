...
const http = require('http'); 
      ...
      MongoClient = require('mongodb').MongoClient, //Requierir el módulo mongodb
      ...
      //Definir la ruta y nombre de la base de datos utilizando mongodb.
        connection = mongoose.connect('mongodb://localhost/agenda_db', { useNewUrlParser: true}, function(error){//definir y establecer la base de datos a utilizar
           if(error){ //Verificar si existe error al conectarse a mongodb
           	 console.log(error.name +" "+ error.message); //Mostrar mensaje de error
           }else{
...
const Server = http.createServer(app) //Crear el servidor a través del módulo http
